...
import Test.Common
import Test.Dscp.Core.Instances (genStudentSignedSubmissions)
            ...
            sqliteProperty $
              \( delayedGen
                 ...
                 -> (student, sigSubmissions@(sigSubmissiontoDel :| _))
                  ...
                  prepareAndCreateSubmissions sigSubmissions
                  before <- getAllSubmissions student
                  ...
                  after <- getAllSubmissions student
                  let expected =
                          filter (\s -> sHash s /= submissionToDelH) before

                  return $ sortOn sHash after === sortOn sHash expected
