...
getStudentCourses :: MonadQuery cmd be hdl m => Id Student -> m [Id Course]
...
enrollStudentToCourse
    :: (MonadQuery cmd be hdl m, WithinWriteTx)
...
getStudentTransactions :: MonadQuery cmd be hdl m => Id Student -> m [PrivateTx]
...
getProvenStudentTransactions
    :: forall cmd be hdl m.
       (MonadQuery cmd be hdl m, WithinTx)
    => GetProvenStudentTransactionsFilters
...
getAllNonChainedTransactions :: MonadQuery cmd be hdl m => m [PrivateTx]
...
getLastBlockIdAndIdx
    :: (MonadQuery cmd be hdl m, HasLens' ctx (KeyResources EducatorNode))
...
createPrivateBlock
    :: (MonadQuery cmd be hdl m, HasLens' ctx (KeyResources EducatorNode), WithinWriteTx)
...
isEnrolledTo :: MonadQuery cmd be hdl m => Id Student -> Id Course -> m Bool
...
createCourse :: (MonadQuery cmd be hdl m, WithinWriteTx) => CourseDetails -> m (Id Course)
...
getCourseSubjects :: MonadQuery cmd be hdl m => Course -> m [Subject]
...
existsCourse :: MonadQuery cmd be hdl m => Id Course -> m Bool
existsCourse = existsWithPk (esCourses es)

existsStudent :: MonadQuery cmd be hdl m => Id Student -> m Bool
existsStudent = existsWithPk (esStudents es)

existsSubmission :: MonadQuery cmd be hdl m => Id Submission -> m Bool
existsSubmission = existsWithPk (esSubmissions es)

createStudent :: (MonadQuery cmd be hdl m, WithinWrite) => Student -> m (Id Student)
...
getTransaction :: MonadQuery cmd be hdl m => Id PrivateTx -> m (Maybe PrivateTx)
